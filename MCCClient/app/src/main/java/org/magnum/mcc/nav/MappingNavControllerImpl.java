...
public class MappingNavControllerImpl implements NavController {
    ...
    private static String IMG_PATH = "/image/edge/";
     ...
     */
    private WeakReference<MapRouteActivity> activity_ = null;
    ...
    public void getPictureOnPath(String floorplanId, FloorplanEdge edge,
                                EdgeImageListener imgListener) {

        targetFloorplanId_ = floorplanId;
        targetUrl_ = host_ + urlBase_ + IMG_PATH + targetFloorplanId_ + "/"
                + edge.getStart().getId() + "/" + edge.getEnd().getId();

        Log.d(TAG, "url: " + targetUrl_);
        new PathPictureAsyncTask().execute(imgListener);
    }
    ...
    private class FloorplanNavDataAsyncTask extends
        ...
        protected FloorplanNavigationData doInBackground(FloorplanListener... fpListeners) {
            ...
            try {
                ...
                Log.d(TAG, "Inside FloorplanNavData: " + fpNavContent);

                JSONObject fpNavJSON = new JSONObject(fpNavContent);
                Log.d(TAG, "JSON: " + fpNavJSON);
    ...
    /**
     * The AsyncTask responsible for retrieving the path picture related with the path
     */
    private class PathPictureAsyncTask extends
            AsyncTask<EdgeImageListener, Integer, Void> {

        Bitmap bmp;

        @Override
        protected Void doInBackground(EdgeImageListener... imgListeners) {

            DefaultHttpClient client = new DefaultHttpClient();
            HttpGet get = new HttpGet(targetUrl_);
    ...
    private Floorplan extractFloorplan(JSONObject fpNavJSON) throws JSONException {
        ...
        for (int i = 0; i < edges.length(); ++i) {
            ...
            // only weight, no length, is this correct? should check with server side
            Double fpEdgeLen = (Double) fpEdge.get("weight");
