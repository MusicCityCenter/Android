...
public class EventListforthreedays extends Activity {
    ...
    private TextView textviewYestoday;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Intent i = getIntent();
        currentLocID = i.getStringExtra("myLocationId");
        ...
        textviewYestoday = (TextView) view1.findViewById(R.id.textView1);
        textviewToday = (TextView) view2.findViewById(R.id.textView1);
        textviewTomorrow = (TextView) view3.findViewById(R.id.textView1);

        int port = 80;
        String server = "http://0-1-dot-mcc-backend.appspot.com";
        getToday();
        // String baseUrl = "/mcc/events/full-test-1/on/" + month + "/" + day+ "/" + year;
        String baseUrl = "/mcc/events/full-test-1/on/5/9/2014";
        String url = server + baseUrl;

        Log.d(TAG, "log into:"+url);
		/*
        ...
        listviewToday.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
                ...
                try {
                    ...
                    intent.putExtra("myLocationId", currentLocID);
	...
	private class Downloadjsontoday extends AsyncTask<String, Integer, String> {
		...
		protected String doInBackground(String... params) {
			try {
				Log.d("Downloadjsontoday", "start connect"+ params[0]);
		...
		protected void onPostExecute(String result) {
			try {
				Log.d("Downloadjsontoday", "connect ends");
            ...
            if(eventnametoday.size() == 0)
                textviewToday.setText("No event on selected day.");
		}
	}
	...
	private class Downloadjsonyesterday extends
		...
		protected String doInBackground(String... params) {
			try {
				Log.d("Downloadjsonyesterday", "start connect:"+ params[0]);
				StringBuilder sb = new StringBuilder();
				HttpClient client = new DefaultHttpClient();
				HttpParams httpParams = client.getParams();
				// set Timeout
		...
		protected void onPostExecute(String result) {
			try {
				Log.d("Downloadjsonyesterday", "connect ends:" + result);
				array = new JSONArray(result);
				for (int i1 = 0; i1 < array.length(); i1++) {
					JSONObject obj = array.getJSONObject(i1);
					String startTime = obj.getString("startTime");
            ...
            if(eventnameyesterday.size() == 0)
                textviewYestoday.setText("No event on selected day.");
		}
	}
	...
	private class Downloadjsontomorrow extends
		...
		protected String doInBackground(String... params) {
			try {
				Log.d("Downloadjsontomorrow", "start connect"+ params[0]);
				StringBuilder sb = new StringBuilder();
				HttpClient client = new DefaultHttpClient();
				HttpParams httpParams = client.getParams();
				// set Timeout
		...
		protected void onPostExecute(String result) {
			try {
				Log.d("Downloadjsontomorrow", "connect ends"+ array.length());
				array = new JSONArray(result);
				for (int i1 = 0; i1 < array.length(); i1++) {
					JSONObject obj = array.getJSONObject(i1);
					String startTime = obj.getString("startTime");
            ...
            if(eventnametomorrow.size() == 0)
                textviewTomorrow.setText("No event on selected day.");
		}
	}
